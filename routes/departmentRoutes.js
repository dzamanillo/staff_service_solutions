...
const departmentBudget = (id) => {
	const sql = `SELECT department.department_name AS Department, SUM(salary) AS Budget FROM roles LEFT JOIN department ON department.id = roles.department_id WHERE department.id = ?`;
	const params = id;
